defmodule Gealts do
  ...
  """
	...
	Should takes a list and return a number.
	      ...
	      For example: (fn [a, b, c, d] -> :erlang.abs((a + 2 * b + 3 * c + 4 * d) - 30) end)                      
  """
	...
	@doc """
	Performs n number of itererations, altering chromosome population in the process.
  """
  ...
  """
	...
	defp do_start(config = %{input: []}) do
		input = for _ <- 1..config[:chromes] do
			MathUtils.random_values_list(config[:genes], config[:min_val], config[:max_val])
		end
